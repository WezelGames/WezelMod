...
import me.waeal.wezelmod.objects.ItemComponent;
...
public class ProfileServices {
    public void showInventory(String player) {
        Minecraft.getMinecraft().displayGuiScreen(new InventoryGui(player));
    }
}
...
class Gui extends WindowScreen {
    ...
    private void drawMenu() {
        ...
        UIComponent header = new UIText("WezelMod").setColor(new Color(0xC75EC4)).setX(new PixelConstraint(getWindow().getWidth() * 0.03f)).setY(new PixelConstraint(getWindow().getHeight() * 0.03f)).setChildOf(background);
        header.setTextScale(new PixelConstraint(getWindow().getHeight() * 0.04f / header.getHeight()));

        addMenuButton("Inventory", getWindow().getHeight() * 0.08f + 5, () -> Minecraft.getMinecraft().displayGuiScreen(new InventoryGui(name)), background);
        addMenuButton("Catacombs", getWindow().getHeight() * 0.08f + 28, () -> Minecraft.getMinecraft().displayGuiScreen(new CatacombsGui(name)), background);
        addMenuButton("Pets", getWindow().getHeight() * 0.08f + 51, () -> Minecraft.getMinecraft().displayGuiScreen(new PetsGui(name)), background);
...
class CatacombsGui extends Gui {
    public CatacombsGui(String name) {
        super(name);
    }
}

class PetsGui extends Gui {
    public PetsGui(String name) {
        super(name);
    }
}
